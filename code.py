# rate_control_joystick_xy_variable_speed_improved.py
...
pwm_x = pwmio.PWMOut(board.GP13, frequency=50)  # X axis (pan)
x_servo = servo.Servo(pwm_x, min_pulse=650, max_pulse=2350)
pwm_y = pwmio.PWMOut(board.GP12, frequency=50)  # Y axis (tilt)
y_servo = servo.Servo(pwm_y, min_pulse=650, max_pulse=2350)
pwm_launch = pwmio.PWMOut(board.GP11, frequency=50)  # Launch
launch_servo = servo.Servo(pwm_launch, min_pulse=650, max_pulse=2350)

# Set initial angles at 90°
x_servo.angle = 90
y_servo.angle = 90
launch_servo.angle = 90
# Disable servos after positioning to prevent startup jitter
x_servo.angle = None
y_servo.angle = None
launch_servo.angle = None

# Setup Joysticks on Pico Analog Pins (remember a joystick is just two potentiometers)
x_axis = analogio.AnalogIn(board.A1)  # GP27
y_axis = analogio.AnalogIn(board.A0)  # GP26
...
while True:
    ...
    if abs(current_x_angle - last_x_angle_sent) >= servo_update_threshold:
        x_servo.angle = round(current_x_angle) # Send angle command to servo
    ...
    if x_servo_enabled and (time.monotonic() - last_x_movement_time) > servo_timeout:
        x_servo.angle = None  # Disables the PWM signal, effectively shutting off the servo
    ...
    if abs(current_y_angle - last_y_angle_sent) >= servo_update_threshold:
        y_servo.angle = round(current_y_angle)  # Send angle command to servo
    ...
    if button.pressed:  # If pressed, launch by immediately moving to 0°
        ...
        for i in range(91):  # And after launch, slowly move back to 90°. If I didn't move slowly I found sometimes the code reset.
            launch_servo.angle = i
            time.sleep(0.01)
        launch_servo.angle = None  # Disable launch servo to prevent jitter